...
GuiFieldEvents(iniFile)
{	
	global EventEnabled
	global EventFile
	global EventNicks
	
	IniRead, EventEnabled, %iniFile%, Event, EventEnabled
	IniRead, EventFile, %iniFile%, Event, EventFile
	
	if EventEnabled
	{
		Gui, Add, CheckBox, Checked vEventEnabled, Enable custom event sound
	}
	else 
	{
		Gui, Add, CheckBox, vEventEnabled, Enable custom event sound
	}
	
	Gui, Add, Text,, Edit Assets/events.txt for custom event triggers
	
	Gui, Add, ListBox, ReadOnly vEventNicks w290 h80
	Loop, Read, Assets\events.txt
	{
		GuiControl,, EventNicks, %A_LoopReadLine%
	}
		
	Gui, Add, Button, XP290 YP0 Default gEditEvent, Edit
		
	Gui, Add, DropDownList, vEventFile w320 XP-290 YP75
	Loop, files, Sounds\*.*
	{
		GuiControl,, EventFile, %A_LoopFileFullPath%
	}
	GuiControl, ChooseString, EventFile, %EventFile%
